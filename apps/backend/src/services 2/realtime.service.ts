...
export class RealtimeService extends EventEmitter {
  ...
  private maxEventListeners = 50; // Prevent memory leaks from too many listeners
  ...
  constructor(server: HttpServer) {
    ...
    this.io = new SocketIOServer(server, {
      ...
      allowEIO3: true,
      // Add connection limits to prevent memory exhaustion
      maxConnections: config.isProduction ? 1000 : 100,
    ...
    if (config.redis?.url) {
      try {
        ...
        this.redis.on('ready', () => {
          logger.info('Redis connection established for real-time service');
    ...
    // Monitor this EventEmitter for potential leaks
    MemoryManager.monitorEventEmitter(this, 'RealtimeService');
  ...
  private setupRedisSubscriptions(): void {
    ...
    this.redis.subscribe('castmatch:realtime:*').catch((error) => {
      logger.error('Redis subscription failed:', error);
    });
  ...
  private startHealthCheck(): void {
    this.healthCheckInterval = MemoryManager.createSafeInterval(() => {
      const now = Date.now();
      ...
      this.connectedUsers.forEach((user, socketId) => {
        if (now - user.lastSeen.getTime() > staleThreshold) {
          ...
          user.subscriptions.forEach(channel => {
            ...
            // Remove empty channel subscriptions
      ...
      if (memUsage.heapUsed > 100 * 1024 * 1024) { // > 100MB
        logger.warn('High memory usage detected in realtime service', {
  ...
  /**
   * Start cleanup process for memory management
   */
  private startCleanupProcess(): void {
    this.cleanupInterval = MemoryManager.createSafeInterval(() => {
      ...
      emptyChannels.forEach(channel => this.subscriptions.delete(channel));

      // Force garbage collection if available (for development)
      if (global.gc && !config.isProduction) {
        global.gc();
      }
...
export class SSEService {
  ...
  public handleConnection(req: Request, res: Response): void {
    ...
    // Setup keep-alive ping with cleanup
    const pingInterval = setInterval(() => {
      try {
        this.sendEvent(res, 'ping', { timestamp: new Date().toISOString() });
      } catch (error) {
        logger.error('Failed to send SSE ping, cleaning up connection:', error);
