...
// import sgMail from '@sendgrid/mail'; // Removed - using Resend instead
...
export class EmailService {
  ...
  private async sendEmailInternal(options: EmailOptions): Promise<void> {
    try {
      ...
      switch (this.provider) {
        // SendGrid case removed - using Resend instead
  ...
  /**
   * Send OTP for progressive signup
   */
  async sendOTPEmail(to: string, otp: string): Promise<void> {
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
          .content { background: white; padding: 30px; border: 1px solid #e0e0e0; border-radius: 0 0 10px 10px; }
          .otp-box { background: #f8f9fa; border: 2px solid #667eea; border-radius: 8px; padding: 30px; text-align: center; margin: 30px 0; }
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p>We're excited to have you join CastMatch! Please verify your email address using the code below:</p>
            
            <div class="otp-box">
              <p style="margin: 0; font-size: 16px; color: #666;">Your verification code</p>
              <div class="otp-code">${otp}</div>
              <p style="margin: 0; font-size: 14px; color: #888;">Enter this code to complete your registration</p>
            </div>
            ...
            <p>Welcome to Mumbai's premier AI-powered casting platform!</p>
          </div>
          <div class="footer">
            <p>© ${new Date().getFullYear()} CastMatch. All rights reserved.</p>
            <p>This is an automated email. Please do not reply.</p>
    ...
    const text = `
      CastMatch - Email Verification
      
      Complete your account setup by entering this verification code:
    ...
    await this.sendEmail({
      to,
      subject: `${otp} is your CastMatch verification code`,
      html,
      text,
      priority: 'high',
      categories: ['otp', 'verification', 'signup'],
      metadata: {
