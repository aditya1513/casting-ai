  ...
  return function MockMainContent({ 
    ...
    return (
      <div data-testid="main-content">
        ...
        <button onClick={() => onQuickAction?.('find-male-lead')}>Quick Action: Find Male Lead</button>
        <button onClick={() => onQuickAction?.('schedule-auditions')}>Quick Action: Schedule Auditions</button>
        <button onClick={() => onQuickAction?.('analyze-script')}>Quick Action: Analyze Script</button>
        <button onClick={() => onQuickAction?.('budget-planning')}>Quick Action: Budget Planning</button>
      </div>
    )
  }
})
...
jest.mock('../components/Shared/LiveRegion', () => {
  return function MockLiveRegion({ message }: any) {
    return <div data-testid="live-region" aria-live="polite">{message}</div>
  }
})
...
describe('DashboardPage - Quick Actions Auto-send', () => {
  ...
  describe('Functional Testing', () => {
    it('should auto-send message when "Find Male Lead" quick action is clicked', async () => {
      ...
      expect(getByTestId('input-value')).toHaveTextContent('Find me suitable male lead actors for Mumbai Dreams')
    ...
    it('should auto-send message for all quick action types', async () => {
      const quickActions = [
        { button: 'Quick Action: Schedule Auditions', message: 'Help me schedule auditions for this week' },
        { button: 'Quick Action: Analyze Script', message: 'Analyze the script and suggest character requirements' },
        { button: 'Quick Action: Budget Planning', message: 'Help me plan the casting budget for this project' }
      ]
  ...
  describe('Error Handling Testing', () => {
    it('should handle API failure gracefully', async () => {
      ...
      expect(getByTestId('loading-state')).toHaveTextContent('false')
      expect(getByTestId('live-region')).toHaveTextContent('Error occurred while sending message. Please try again.')
    })
    ...
    it('should fall back from authenticated to public endpoint on auth failure', async () => {
      ...
      expect(mockFetch).toHaveBeenNthCalledWith(1, '/api/conversations/protected', expect.any(Object))
      expect(mockFetch).toHaveBeenNthCalledWith(2, '/api/conversations/messages', expect.any(Object))
  ...
  describe('State Management Testing', () => {
    ...
    it('should update live region announcements during quick action', async () => {
      ...
      await waitFor(() => {
        expect(getByTestId('live-region')).toHaveTextContent(expect.stringContaining('Using public agents'))
      })
      
      // Should announce when response received
      await waitFor(() => {
        expect(getByTestId('live-region')).toHaveTextContent(expect.stringContaining('AI response received'))
      })
    })
  })
