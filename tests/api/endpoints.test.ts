/**
 * API Endpoint Tests
 * Comprehensive testing for all API endpoints with error handling
 */
...
describe('API Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    for (const userData of users) {
      ...
      if (userData.role === 'ACTOR') {
        ...
        await prisma.castingDirector.create({
          data: {
            userId: user.id,
            firstName: userData.firstName,
            lastName: userData.lastName,
    ...
    describe('POST /api/auth/register', () => {
      ...
      it('should return 409 for duplicate email', async () => {
        const response = await request(app)
          .post('/api/auth/register')
          .send({
            email: testUsers.actor.email, // Already exists
            password: 'ValidPass123!',
            confirmPassword: 'ValidPass123!',
            firstName: 'Duplicate',
            lastName: 'Email',
      ...
      it('should return 400 for missing required fields', async () => {
        const response = await request(app)
          ...
          .send({
            ...
            // Missing password, firstName, lastName, role, acceptTerms
          });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle unsupported HTTP methods', async () => {
      const response = await request(app)
        .patch('/api/auth/login'); // Unsupported method
    ...
    it('should validate field lengths', async () => {
      const response = await request(app)
        ...
        .send({
          ...
          password: 'ValidPass123!',
          confirmPassword: 'ValidPass123!',
          firstName: 'A'.repeat(256), // Very long first name
          lastName: 'User',
          role: 'ACTOR',
          acceptTerms: true,
        });
