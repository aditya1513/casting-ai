...
class AIMatchingService {
  ...
  async findSimilarTalents(
    ...
    try {
      ...
      const vectorData = await vectorService.getTalentVector(talentId);
      if (!vectorData || !vectorData.values) {
        throw new AppError('Talent embedding not found', 404);
      }

      // Search for similar talents
      const similarResults = await vectorService.searchSimilarTalents(
        vectorData.values,
        {
  ...
  async updateAllTalentEmbeddings(
    ...
    try {
      ...
      for (let offset = 0; offset < totalCount; offset += batchSize) {
        ...
        if (records.length > 0) {
          await vectorService.batchUpsertEmbeddings(records);
