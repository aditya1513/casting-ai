...
generate_dashboard() {
    cat > "$TEMP_FILE" << EOF
# ⚡ RAPID PROGRESS MONITOR - REAL-TIME

**🔴 LIVE UPDATES** every 1 second | $(if [[ "$(check_frontend)" == *"Running"* ]] && [[ "$(check_backend)" == *"Running"* ]]; then echo "✅ ALL SYSTEMS OPERATIONAL"; else echo "⚠️ SERVICE STARTUP REQUIRED"; fi)

## ⚡ LIVE STATUS [Time: $CURRENT_TIME] Next Update: $NEXT_UPDATE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
**Backend:** $(check_backend) | **Frontend:** $(check_frontend) | **Active Agents:** $(echo "$recent_files" | wc -l | xargs) files being worked on
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- **Chat API:**         $(if [[ "$(check_backend)" == *"Running"* ]]; then echo "[████████░░] 80% (Backend healthy)"; else echo "[███░░░░░░░] 30% (Backend down)"; fi)
...
- **Frontend Status:** $(check_frontend)
- **Auto-refresh:** ⚡ Every 1 second

## 🔥 QUICK WINS POSSIBLE IN NEXT 5 MIN
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

$(if [[ "$(check_frontend)" != *"Running"* ]]; then
echo "- **HIGH PRIORITY:** Start Next.js dev server (\`cd frontend && npm run dev\`)"
fi)
$(if [[ "$(check_backend)" != *"Running"* ]]; then
echo "- **HIGH PRIORITY:** Start backend server (\`npm run dev\`)"
fi)
...
\`\`\`bash
# Auto-detecting backend port (5006/5002/process)
Backend Status: $(check_backend)
\`\`\`

**FRONTEND STATUS:**
\`\`\`bash  
# Auto-detecting frontend port (3000/3001/Next.js process)
Frontend Status: $(check_frontend)
\`\`\`
...
$(if [[ "$(check_frontend)" != *"Running"* ]] || [[ "$(check_backend)" != *"Running"* ]]; then
  echo "- 🎯 STARTUP NEEDED (Priority #1)"
  if [[ "$(check_frontend)" != *"Running"* ]]; then echo "  - Frontend server needed"; fi
  if [[ "$(check_backend)" != *"Running"* ]]; then echo "  - Backend server needed"; fi
else
  echo "- ✅ All services running - Ready for API work"
fi)
...
- **CURRENT FOCUS:** $(if [[ "$(check_frontend)" == *"Running"* ]] && [[ "$(check_backend)" == *"Running"* ]]; then echo "✅ Both Services Running"; else echo "🚨 SERVICE STARTUP NEEDED"; fi)
- **Backend:** $(check_backend)  
- **Frontend:** $(check_frontend)
- **Next Action:** $(if [[ "$(check_frontend)" != *"Running"* ]]; then echo "Launch Next.js (cd frontend && npm run dev)"; elif [[ "$(check_backend)" != *"Running"* ]]; then echo "Start backend server"; else echo "API integration testing"; fi)
